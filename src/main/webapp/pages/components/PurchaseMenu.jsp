...
<nav class="col-sm-3 col-md-2 d-none d-sm-block bg-light sidebar">
  <ul class="nav nav-pills flex-column">
    <c:if test="${sessionScope.USER != null && sessionScope.USER.role=='CLIENT'}">
      ...
      <li class="nav-item">
        <a class="nav-link" href="<c:url value="/action?name=order_list&status=SUBMITTED"/>">
    ...
    <c:if test="${sessionScope.USER != null && sessionScope.USER.role=='ADMIN'}">"
      <li class="nav-item">
        <a class="nav-link" href="<c:url value="/action?name=order_list&status=COMPLETED&user=${requestScope.user}"/>">
          <fmt:message key="purchasemenu.completed"/></a>
      </li>
      <li class=" nav-item">
        <a class="nav-link" href="<c:url value="/action?name=order_list&status=SUBMITTED&user=${requestScope.user}"/>">
          <fmt:message key="purchasemenu.submitted"/></a>
        <a class="nav-link"
           href="<c:url value="/action?name=order_list&status=REJECTED&user=${requestScope.user}"/>"><fmt:message
          key="purchasemenu.rejected"/></a>
      </li>
    </c:if>
  </ul>
